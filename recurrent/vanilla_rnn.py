...
class RNN():
	def __init__(self, n_neurons, n_layers, batch=1, cont=False, epochs=100, size=45, activation='relu', loss=tf.keras.losses.mean_squared_error):
		if cont:
			try:
				self.model = self.load(f'./models/sort_net_{size}.rnn')
			except:
	...
	def build(self, input_size):
		# input layer
		self.model.add(layers.(units=5, input_dim=input_size, activation=self.activation))
		# hidden layers
		for _ in range(self.n_layers):
			self.model.add(layers.(units=self.n_neurons, activation=self.activation))
		# output layer
		self.model.add(layers.(units=input_size, activation=tf.math.softplus, kernel_regularizer=tf.keras.kernel_regularizer.L1L2(l1=.01, l2=.01)))
		# build neural net
	...
	def save(self, size):
		tf.keras.models.save_models(self.model, f'./models/sort_net_{size}.rnn')
		return self.model

	def load(self, size):
		return tf.keras.models.load_model(f'./models/sort_net_{size}.rnn')